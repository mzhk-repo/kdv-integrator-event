...
- UI завжди створює XLSX на Google Drive для записів з `856$y = "Файл"`. Email — окрема вимкнена за замовчуванням опція; SQLite для UI-експорту не використовується в жодному режимі. Один export може працювати одночасно в одному service process; паралельний запит отримає `409`.
...
printf "%s\n" \
  ...
  "EXPORT_MARC_MAPPING_PATH=${EXPORT_MARC_MAPPING_PATH:-}" \
  "EXPORT_DICTIONARIES_PATH=${EXPORT_DICTIONARIES_PATH:-}" \
