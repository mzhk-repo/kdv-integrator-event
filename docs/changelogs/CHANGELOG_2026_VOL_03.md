...
## 2026-05-26 — Koha ExportConfig і RuntimeOptions (Завдання 0.3)

- **Context:** Для завершення Фази 0 Koha Export потрібен централізований config layer, який читає env за тим самим SOPS/age bootstrap pattern, що й основний `src/config.py`, але не змішує env-конфіг із runtime flags.
- **Change:** Додано `src/export_module/config.py` з `ExportConfig`, `RuntimeOptions`, `ConfigValidationError`, `from_env()`, `validate()` і `parse_runtime_options()`. Env bootstrap підтримує `ORCHESTRATOR_ENV_FILE`, `SERVER_ENV=dev/prod`, plain `env.<env>`, encrypted `env.<env>.enc` через `sops --decrypt --age-key-file` і fallback `.env` без override вже встановлених env. `EXPORT_DRY_RUN` не читається; dry-run доступний тільки через CLI `--dry-run`. `.env.example` доповнено export block, включно з `EXPORT_DB_PATH`.
- **Verification:** `python3 -m py_compile src/export_module/config.py tests/test_export_config.py`; `python3 -m pytest tests/test_export_schema.py tests/test_export_repository.py tests/test_export_config.py -q` -> `14 passed`; `rg -n "EXPORT_DRY_RUN|SMTP_|GDRIVE_SERVICE_ACCOUNT_FILE" src/export_module/config.py tests/test_export_config.py .env.example` підтвердив, що export config не читає заборонені env; `git diff --check -- src/export_module/config.py tests/test_export_config.py .env.example docs/changelogs/CHANGELOG_2026_VOL_03.md` без whitespace-зауважень.
- **Risks:** `validate()` створює `EXPORT_GDRIVE_ROOT_PATH` тільки якщо шлях всередині `/mnt/drive`; у production це залежить від фактичного rclone mount і прав контейнера. Реальні Graph credentials не перевірялися, тільки local validation contract без витоку secret-значень.
- **Rollback:** Видалити `src/export_module/config.py`, `tests/test_export_config.py`, прибрати export block з `.env.example` і цей changelog-запис.


## 2026-05-26 — Koha Export MARC mapping і словники (Завдання 1.1)

- **Context:** Після завершення Фази 0 Koha Export потрібен декларативний mapping contract для XLSX-колонок, static/spec columns і перекодування Koha Authorized values без хардкоду в Python.
- **Change:** Додано `config/marc_mapping.yaml` з колонками `ID Запису`, `Назва книги`, `Тип документа`, static columns `Бібліотека-отримувач` і `Статус імпорту`, а також `config/export_dictionaries.yaml` зі словниками `itemtypes` (`BOOK -> Книга`, `BK -> Книга`, `CR -> Періодика`, `VM -> Відеоматеріал`) і `ccodes` (`FICTION -> Художня література`, `SCIENCE -> Наукова література`). Додано `src/export_module/marc/mapping_loader.py` з dataclass contract, `MappingLoader`, `MappingValidationError`, валідацією dictionary references і `required_columns`, а також focused тести.
- **Verification:** `python3 -m py_compile src/export_module/marc/__init__.py src/export_module/marc/mapping_loader.py tests/test_export_mapping_loader.py`; `python3 -m pytest tests/test_export_mapping_loader.py -q` -> `4 passed`; `python3 -m pytest tests/test_export_schema.py tests/test_export_repository.py tests/test_export_config.py tests/test_export_mapping_loader.py -q` -> `18 passed`; `git diff --check -- requirements.txt docs/changelogs/CHANGELOG_2026_VOL_03.md` і `git diff --no-index --check /dev/null ...` для нових файлів без whitespace-зауважень.
- **Risks:** Static columns поки лише завантажуються mapping layer-ом; фактичне додавання їх у XLSX буде у задачах MARCParser/XLSXGenerator. Список Authorized values є стартовим contract-ом і має оновлюватися разом із mapping при змінах Koha.
- **Rollback:** Видалити `config/marc_mapping.yaml`, `config/export_dictionaries.yaml`, `src/export_module/marc/`, `tests/test_export_mapping_loader.py`, прибрати `PyYAML` з `requirements.txt` і цей changelog-запис.


## 2026-05-26 — Koha Export JSON logger і run_id correlation (Завдання 1.2)

- **Context:** Після declarative mapping export-модулю потрібен локальний JSON-compatible logger, який додає `run_id` до кожного log-рядка і не допускає витоку Graph secrets, tokens або legacy event names.
- **Change:** Додано `src/export_module/observability/logger.py` і package init. Logger використовує stdlib `logging`, `contextvars` для `run_id`, `ExportJsonFormatter`, `configure_export_logging()`, `get_export_logger()`, `set_run_id()`/`reset_run_id()`/`get_run_id()`, sanitizer для `secret`/`token`/`password`/`authorization` полів і текстових шаблонів, а також normalization legacy events `gdrive_upload_* -> gdrive_copy_*` і `smtp_* -> graph_email_*`. Додано focused тести `tests/test_export_logger.py`.
